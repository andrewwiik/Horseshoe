...
%subclass CCXSettingsPolusViewController : UITableViewController
%property (nonatomic, retain) _UIVisualEffectLayerConfig *primaryEffectConfig;
%property (nonatomic, retain) NSObject *prefsController;
...
- (void)viewDidLoad {
   ...
   // [self.tableView setEditing:YES animated:NO];
    ...
    self.enabledIdentifiers = [(PLAppsController *)self.prefsController visibleAppsForViewMode:self.viewMode];
    self.disabledIdentifiers = [(PLAppsController *)self.prefsController hiddenAppsForViewMode:self.viewMode];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([switchIdentifier hasPrefix:ACTION_PREFIX]) cell.textLabel.text = nameForActionButton(switchIdentifier, NO);
	else if ([switchIdentifier hasPrefix:FS_PREFIX]) cell.textLabel.text = [panel titleForSwitchIdentifier:[switchIdentifier stringByReplacingOccurrencesOfString:FS_PREFIX withString:@""]];
	else cell.textLabel.text = [[[NSClassFromString(@"ALApplicationList") sharedApplicationList] applications] objectForKey:switchIdentifier];

	cell.imageView.image = [[[NSClassFromString(@"PLAppsController") sharedInstance] glyphOfSize:PLGlyphSizeSmall forAppIdentifier:switchIdentifier withViewMode:self.viewMode] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    cell.iconColor = [panel primaryColorForSwitchIdentifier:[switchIdentifier stringByReplacingOccurrencesOfString:FS_PREFIX withString:@""]] ? [panel primaryColorForSwitchIdentifier:[switchIdentifier stringByReplacingOccurrencesOfString:FS_PREFIX withString:@""]] : [UIColor grayColor];
    return cell;
}
...
- (void)_flushSettings
{
	...
	runOnMainQueueWithoutDeadlocking(^{
	    [(PLAppsController *)self.prefsController setVisibleApps:self.enabledIdentifiers forViewMode:self.viewMode];
	    [(PLAppsController *)self.prefsController  setHiddenApps:self.disabledIdentifiers forViewMode:self.viewMode];

	    [(PLAppsController *)self.prefsController saveInfo];
	});
}
